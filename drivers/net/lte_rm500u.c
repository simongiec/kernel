 ...
 *
 * Rockchip rm500u driver for 4g modem
 */
...
static int modem_poweron_off(int on_off)
{
	...
	if (pdata) {
		if (on_off) {
			...
			if (pdata->power_gpio) {
				printk("=====enter to pull up power_gpio===\r\n");
...
static int lte_probe(struct platform_device *pdev)
{
	...
	printk("====go to lte_probe 3333333====xxx==\n");
	return 0;
err:
	gpdata = NULL;
	return ret;
...
static const struct of_device_id modem_platdata_of_match[] = {
	{ .compatible = "quectel,rm500u" },
	{ }
};
MODULE_DEVICE_TABLE(of, modem_platdata_of_match);

static struct platform_driver rm500u_driver = {
	.probe		= lte_probe,
	.remove		= lte_remove,
	.suspend	= lte_suspend,
	.resume		= lte_resume,
...
static int __init rm500u_init(void)
{
	int ret;
	...
	return platform_driver_register(&rm500u_driver);
}

static void __exit rm500u_exit(void)
{
	platform_driver_unregister(&rm500u_driver);
}

late_initcall(rm500u_init);
module_exit(rm500u_exit);

MODULE_AUTHOR("xuxuehui <xxh@rock-chips.com>");
MODULE_AUTHOR("Alex Zhao <zzc@rock-chips.com>");
MODULE_DESCRIPTION("rm500u lte modem driver");
MODULE_LICENSE("GPL");
